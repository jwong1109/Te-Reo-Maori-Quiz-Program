# Trial 2: Getting an answer input of English months by using an entry field
...
def hard_ask():
    ...
    question_label.grid(column=3, row=1, columnspan=3,
                        pady=20)
    get_answer("Hard", ask_details[0])


# Get answer input
def get_answer(difficulty, correct):
    correct_month = correct[0]
    if difficulty == "Easy":
        ans_entry = Entry(root, font=("Arial", 20))
        ans_entry.grid(column=4, row=2, ipadx=10, sticky=W, ipady=10)
